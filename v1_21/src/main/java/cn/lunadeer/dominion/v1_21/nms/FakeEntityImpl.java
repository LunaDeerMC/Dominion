...
/**
 * Fake entity implementation for Minecraft 1.21.
 * <p>
 * Metadata indices for Display entities in 1.21 (adds pos/rot interpolation at index 10):
 * <pre>
 *  8: INT   - interpolation start delay
 *  9: INT   - interpolation duration
 * 10: INT   - pos/rot interpolation duration (NEW)
 * 11: VEC3  - translation
 * 12: VEC3  - scale
 * 13: QUAT  - left rotation
 * 14: QUAT  - right rotation
 * 15: BYTE  - billboard
 * 16: INT   - brightness override
 * 17: FLOAT - view range
 * 18: FLOAT - shadow radius
 * 19: FLOAT - shadow strength
 * 20: FLOAT - width
 * 21: FLOAT - height
 * 22: INT   - glow color override
 * BlockDisplay 23: BLOCK_STATE
 * ItemDisplay  23: ITEM_STACK, 24: BYTE (item transform)
 * </pre>
 */
...
public class FakeEntityImpl implements FakeEntity {
    ...
    private ClientboundTeleportEntityPacket createTeleportPacket() {
        ...
        try {
            java.lang.reflect.Constructor<ClientboundTeleportEntityPacket> ctor =
                    ClientboundTeleportEntityPacket.class.getDeclaredConstructor(FriendlyByteBuf.class);
            ctor.setAccessible(true);
            ClientboundTeleportEntityPacket packet = ctor.newInstance(buf);
            buf.release();
            return packet;
        } catch (Exception e) {
            buf.release();
            throw new RuntimeException("Failed to create teleport packet via reflection", e);
        }
    }
